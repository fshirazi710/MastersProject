"""
Vote Session schemas for the API.
This module contains Pydantic models for vote-session-related requests and responses.
"""
...
class VoteSessionCreateRequest(BaseModel):
    """Schema for creating a new vote session (core data)."""
    title: str = Field(
        ..., 
        description="Title of the vote session",
        min_length=3,
        max_length=100,
        examples=["Project Funding Q1 2024"]
    )
    description: str = Field(
        ..., 
        description="Description of the vote session",
        min_length=10,
        max_length=1000,
        examples=["Allocate funding for proposed projects"]
    )
    start_date: str = Field(
        ...,
        description="Start date and time of the vote session (ISO format)",
        examples=["2024-03-01T12:00:00"]
    )
    end_date: str = Field(
        ..., 
        description="End date and time of the vote session (ISO format)",
    ...
    options: List[str] = Field(
        ...
        min_length=2,
        examples=[["Project A", "Project B"], ["1", "3", "5"]]
    )
...
# Rename class
class ExtendedVoteSessionCreateRequest(BaseModel):
    # Rename field and update description
    vote_session_data: VoteSessionCreateRequest = Field(..., description="Core vote session data for the contract")
...
class VoteSessionMetadata(BaseModel):
    # Rename field
    vote_session_id: int # Assuming we get this after contract interaction

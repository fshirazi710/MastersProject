...
async def unjoin_as_holder_transaction(public_key_hex, blockchain_service: BlockchainService = Depends(get_blockchain_service)):
    # Get the latest transaction nonce for the sender's wallet
    nonce = blockchain_service.w3.eth.get_transaction_count(WALLET_ADDRESS)
    
    # Estimate gas required for the transaction execution
    estimated_gas = blockchain_service.contract.functions.unjoinAsHolder(
        public_key_hex
    ).estimate_gas({"from": WALLET_ADDRESS})

    # Build the election creation transaction
    unjoin_as_holder_tx = blockchain_service.contract.functions.unjoinAsHolder(
        public_key_hex
    ).build_transaction({
        'from': WALLET_ADDRESS,
        'gas': estimated_gas,
    ...
    # Sign and send transaction
    signed_tx = blockchain_service.w3.eth.account.sign_transaction(unjoin_as_holder_tx, PRIVATE_KEY)
    tx_hash = blockchain_service.w3.eth.send_raw_transaction(signed_tx.rawTransaction)
    receipt = blockchain_service.w3.eth.wait_for_transaction_receipt(tx_hash)

...
You can view the updated database in vscode with the vscode extensions for viewing sql databases which should let you right-click and press `Open Database` on the `database.db` file in the backend folder, expend the `SQLite Explorer` arrow in te bottom left below `Timeline`, click on the `user` table, then on the play button next to it. If this does not work, try a different extension.
